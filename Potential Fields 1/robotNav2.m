...
function [v, om, rm1, rm2, poses, stuck_timer, rand_goal] = robotNav2(ranges, r_pose, poses, stuck_timer, rand_goal, rm1, rm2, goal, vp, dt)
...
f_ranges = OccForce(ranges, sub_angle);
...
if((pose_var(1) < VAR_THRESH) && ~any(poses(1,:) == 0) && (stuck_timer.time == 0))
    ...
    disp('BANG X');
elseif ((pose_var(2) < VAR_THRESH) && ~any(poses(2,:) == 0) && (stuck_timer.time == 0))
    rand_goal = 500*rand(2,1);
    stuck_timer.time = stuck_timer.top;
    stuck_timer.count = stuck_timer.count + 1;
    stuck_timer = stuck_timer.updateTop();
...
% Update Range Memory
[rm1, rm2] = updateRM(rm1, rm2, ranges, v, om, dt);
