...
angle_increment = 45*(pi/180); % Angular displacement between readings in radians
...
for t = 0:dt:5000
    ...
    line([r_pose(1),laser_xy(8,1)],[r_pose(2),laser_xy(8,2)], 'Color','g') % Heading beam
    for index = [1,7] % To add laser lines to display enter their index in the square bracket
        line([r_pose(1),laser_xy(index,1)],[r_pose(2),laser_xy(index,2)], 'Color','r')
    end
    ...
    v100 = [v, v100(1:end-1)];
    om100 = [om, om100(1:end-1)];
    %figure(2)
    %cla
    %plot(1:100,abs(fft(v100)));
    %figure(1)
